...
class ReportBreakDownExport implements WithEvents, ShouldAutoSize, FromView, WithTitle
{
    protected $status,$user,$start_date,$end_date,$manager,$totalRow;
    public function __construct($request)
    {
       $this->status = $request->exportsStatus;
       $this->user = $request->exportsUser;
	...
	public function view(): View
    {
        ...
        if(!empty($this->user)){
            $query->whereUserId((int)$this->user);
        }
        if(!empty($this->manager)){
            $query->whereManagerId((int)$this->manager);
        }
        ...
        return view('admin.exports.report_breakdown', ['schedule' => $data,'title' => 'BreakDown Report List']);
    }
}
