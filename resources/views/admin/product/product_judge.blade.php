		...
		<div class="modal-content">
			...
			<form id="addForm" method="POST" autocomplete="off">
					...
					<div class="row">
						<div class="col-sm-12">
							<div class="form-group form-group-default">
								<label>Product Judge</label>
								<input id="inspection_point" type="text" class="form-control" placeholder="Product Judge" name="name">
							</div>
						</div>
				...
				<div class="modal-footer no-bd">
					...
					<button type="submit" id="addRowButton" class="btn btn-admin" data-loading-text="Product Judge Register..." data-loading="" data-text="">Add</button>
				</div>
			</form>
		</div>
	</div>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-sm-12">
							<div class="form-group form-group-default">
								<label>Product Judge</label>
								<input id="edit_name" type="text" class="form-control" placeholder="Product Judge" name="name">
							</div>
						</div>
					</div>
					<div class="row">
...
@push("js")
	...
	<script type="text/javascript">
		var delete_url = "{{ route('private.judge.destroy') }}";
		var table;
		...
		$(document).ready(function() {
	    	...
	    	table = $('#record-table').dataTable({
				"oLanguage": {
			        "sEmptyTable": "{{ __("site.no_data", ["attr" => "product judge"]) }}"
			    },
	            "processing": true,
	            "serverSide": true,
	            "ajax": {
	                "url": "{{ route("private.judge.list") }}",
	                "type": "POST",
	                data: function (d) {     
	            ...
	            "columns": [
	                    ...
	                    {   "mRender": function ( data, type, row ) 
	                        {
	                        	var btn = `<div class="form-button-action"> 
		                        				<a href="javascript:void(0);" data-toggle="tooltip" title="Edit" class="btn btn-link btn-primary btn-lg edit-data" data-id="${row.id}"  data-original-title="Edit"> <i class="fa fa-edit"></i> </a>
	               ...
	               createdRow: function( row, data, dataIndex ) {
	                              ...
	                              $(row).find( '.status_edit' ).editable({
	                                    url: "{{ route('private.judge.status') }}",
	        ...
	        $("#addForm").validate({
	            ...
	            submitHandler: function(form) {
	                ...
	                $.ajax({
	                    type: "POST",
	                    url: "{{ route('private.judge.create') }}",
	                    data: data,
	                    dataType: "json",
	        ...
	        $("#editForm").validate({
	            ...
	            submitHandler: function(form) {
	                ...
	                $.ajax({
	                    type: "POST",
	                    url: "{{ route('private.judge.update') }}",
	                    data: data,
	                    dataType: "json",
	                    success: function(data) {
	                        loadButton("#editRowButton");
