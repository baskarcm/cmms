					...
					<div class="row p-2 filter-cont mb-4">
						...
						<div class="col-md-3 pl-0">
							<div class="form-group pt-0">
								...
								<select class="select-filter form-control" data-placeholder="Select a Status" id="user-filter">
	                              	<option value="">All</option>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
	    	...
	    	table = $('#record-table').dataTable({
				"oLanguage": {
			        "sEmptyTable": "{{ __("site.no_data", ["attr" => "pm"]) }}"
			    },
	            "processing": true,
	            "serverSide": true,
	            "ajax": {
	                "url": "{{ route("private.pmreport.list") }}",
	                "type": "POST",
	                data: function (d) {
	                    ...
	                    {   "mRender": function ( data, type, row )
	                        {
	                        	...
	                        	var btn = `<div class="form-button-action"><a href="{{route('private.pm.view', ['key' => ''])}}/${row.key}" data-toggle="tooltip" title="" class="btn btn-link btn-primary btn-lg" data-id="${row.id}" data-original-title="Breakdown view" target="_blank"> <i class="fa fa-eye"></i> </a> <a href="javascript:void(0);" data-toggle="tooltip" title="Export" class="btn btn-link btn-primary btn-lg getData p-2" data-id="${row.id}"> <i class="fa fa-download"></i> </a></div>`;
	                            return btn;
	                         }
	                    }
	                ],
			...
			$('.data-exports').on("click",function(){
	            ...
	            var exportsUser = $('#user-filter').val();
	            var exportsManager = $("#manager-filter").val()
	            var exportsStartDate = startDate;
	            var exportsEndDate = endDate;
	                $.ajax({
	                    type: "POST",
	                    url: "{{ route('private.report.datapm-exports') }}",
	                    data: {exportsStatus:exportsStatus,exportsUser:exportsUser,exportsManager:exportsManager,exportsStartDate:exportsStartDate,exportsEndDate:exportsEndDate},
	                    dataType: "json",
		    ...
		    $('table').on('click','.getData',function(){
		    	var individualId = $(this).attr('data-id');
	                $.ajax({
	                    type: "POST",
	                    url: "{{ route('private.report.individualpm-exports') }}",
