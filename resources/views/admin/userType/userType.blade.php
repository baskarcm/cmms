...
@push("js")
	...
	<script type="text/javascript">
		var delete_url = "{{ route('private.usertype.destroy') }}";
		var table;
		...
		$(document).ready(function() {
	    	...
	    	table = $('#record-table').dataTable({
	            ...
	            "serverSide": true,
	            "ajax": {
	                "url": "{{ route("private.usertype.list") }}",
	                "type": "POST",
	                data: function (d) {     
	            ...
	            "columns": [
	                    ...
	                    {   "mRender": function ( data, type, row ) 
	                        {
	                        	var btn = `<div class="form-button-action"> 
	                        					<a href="" data-toggle="tooltip" title="" class="btn btn-link btn-primary btn-lg set-permission" data-id="${row.id}" data-original-title="Set Permission" target="_blank" style="display:none;"> <i class="fas fa-shield-alt"></i> </a>
	               ...
	               createdRow: function( row, data, dataIndex ) {
	                              ...
	                              $(row).find( '.status_edit' ).editable({
	                                    url: "{{ route('private.usertype.status') }}",
	        ...
	        $("#addForm").validate({
	            ...
	            submitHandler: function(form) {
	                ...
	                $.ajax({
	                    type: "POST",
	                    url: "{{ route('private.usertype.create') }}",
	                    data: data,
	                    dataType: "json",
	        ...
	        $("#editForm").validate({
	            ...
	            submitHandler: function(form) {
	                ...
	                $.ajax({
	                    type: "POST",
	                    url: "{{ route('private.usertype.update') }}",
	                    data: data,
	                    dataType: "json",
	                    success: function(data) {
	                        loadButton("#editRowButton");
            ...
            $("table").on("click", ".edit-data", function(){
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('private.usertype.edit')}}",
                    data: {data:dataId},
                    dataType: "json",
